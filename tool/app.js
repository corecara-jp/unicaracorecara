    ...
    '収入': {
      1: function(imp) {
        return (imp ? '「' + imp + '」を目指す場合、' : '収入目標に向けて、') +
          '転職で年収レンジを引き上げながら、会社員のまま月5〜30万円を稼ぐ副業を並走させる両輪戦略が最も確実です。さらに節税の仕組みを整えることで、月3〜10万円の手残り増加も同時に狙えます。転職の年収ターゲットを決め、不足分を副業でカバーする金額を試算するところから始めましょう。';
      },
      2: function(imp) {
    ...
    '働き方': {
      1: function(imp) {
        return (imp ? '「' + imp + '」を実現するには、' : '働き方を変えるには、') +
          '転職先の働き方条件（リモート・フレックス・残業実態）を選考軸に入れつつ、副業で時間・場所の自由度を高める組み合わせが有効です。本業の環境を変えた上で、週数時間からスタートできる副業でさらに自律的な働き方を設計できます。';
      },
      2: function(imp) {
    ...
    '職場環境': {
      1: function(imp) {
        return (imp ? '「' + imp + '」を目指すなら、' : '職場環境を変えるには、') +
          '転職で環境を一新しつつ、副業で全く別のコミュニティ・評価環境に触れることで、比較軸が生まれ次の選択がしやすくなります。同じ目標を持つ仲間や専属サポーターとのつながりが、現職以外の心理的な拠り所にもなります。';
      },
      2: function(imp) {
    ...
    '仕事内容': {
      1: function(imp) {
        return (imp ? '「' + imp + '」を目指す場合、' : '仕事内容を変えるには、') +
          '転職で職種・業務内容を刷新しつつ、副業で自分の裁量でやりたい仕事に挑戦する組み合わせが有効です。本業で経験を積み、副業で「人の人生に関わる仕事」を実践するサイクルが成長と市場価値を加速させます。';
      },
      2: function(imp) {
    ...
    'スキル': {
      1: function(imp) {
        return (imp ? '「' + imp + '」を達成するには、' : 'スキルを伸ばすには、') +
          '転職で実務経験を積める環境に移りつつ、副業でそのスキルを外部で試す組み合わせが市場価値を最も高めます。「対人折衝能力・マネーリテラシー・投資判断」というAI時代に代替不可能なスキルが、副業の実践を通じて身につきます。';
      },
      2: function(imp) {
    ...
    '生活': {
      1: function(imp) {
        return (imp ? '「' + imp + '」を実現するには、' : '理想の生活を実現するには、') +
          '転職で働き方・勤務地・拘束時間を変えつつ、副業で収入の柔軟性を持つ組み合わせが、生活設計の自由度を最も高めます。節税の仕組みで手残りを増やしながら長期的な資産形成も並走できます。';
      },
      2: function(imp) {
        return (imp ? '「' + imp + '」を実現するには、' : '理想の生活を実現するには、') +
